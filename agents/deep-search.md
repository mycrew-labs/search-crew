...
**文件 1：`<run_root>/deep-search/report.md`** —— 按下面的「报告质量标准」写，这是一份**有论证、有证据强度**的报告，不是 bullet 罗列。

**报告质量标准（6 条，逐条做到）：**

1. **证据类型决定可信度**（领域相关，没有通用权威源）：判一条主张时看「谁有资格说」——参数信 `客观规格`/`独立测评`，体验信 `用户体验`，排名信 `独立测评`。主张与来源类型不匹配 MUST 降级：厂商自夸性能 → 标「厂商宣传·利益相关·待第三方佐证」，博客报参数 → 标「二手」。
2. **独立印证量化**：每条关键结论标「独立印证数（中 X 源 / 英 Y 源）」。去重——多篇引同一新闻稿/同一份测评算 1 源。中英文**各自独立**印证一致 = 强证据。
3. **一手优先**：能追一手（官方规格/原始实测/原始数据）的标「已复核」；仅二手或单源的标「存疑」，不冒充已确认。
4. **冲突显式裁决**：源分歧时 `⚠️ 分歧：源 A=X，源 B=Y`，**并分析原因**（口径差异/地域差异/时效差异），按证据属性给倾向——MUST NOT 和稀泥或无判断并列。
5. **场景化推荐**：分场景给推荐时，每个推荐附「核心支撑数据」（具体指标/数字，不泛泛而谈）。
6. **反信息茧房**：报告 MUST 单独含一段 `## ⚡ 异常 / 少数派 / 反直觉信号`——呈现与主流共识相悖的来源及其理由（如热门型号的隐藏缺陷、冷门选项的意外亮点）。无异常值时显式写「未发现显著异常信号」，**不得省略该段**。
